...
async def find_verified_patient(
    ...
    *,
    ...
    for patient in await emr_client.list_patients(phone=phone):
        if _phone_digits(patient["phone"]) == _phone_digits(phone) and patient["date_of_birth"] == date_of_birth:
            return {
                "status": "verified",
                "patient": {
                    "id": patient["id"],
...
async def register_patient(emr_client: Any, patient: dict[str, Any]) -> dict[str, Any]:
    phone = patient["phone"]
